...
func TestParse(t *testing.T) {
	var tests = []struct {
		...
		{"async function a(){x = await => a++}", "Decl(async function a Params() Stmt({ Stmt(x=(Params(Binding(await)) => Stmt({ Stmt(a++) }))) }))"},
		...
		{"x = async a => {a++}", "Stmt(x=(async Params(Binding(a)) => Stmt({ Stmt(a++) })))"},
		{"x = await => a++", "Stmt(x=(Params(Binding(await)) => Stmt({ Stmt(a++) })))"},
		{"x = a??b", "Stmt(x=(a??b))"},
		{"x = a[b]", "Stmt(x=a[b])"},
...
func TestParseError(t *testing.T) {
	var tests = []struct {
		...
		{"do a++ while", "expected ';' instead of 'while' in expression statement"},
		...
		{"async function a() { class a extends await", "unexpected EOF in expression"},
		...
		{"yield a = 5", "expected ';' instead of 'a' in expression statement"},
		{"if (a) 1 else 3", "expected ';' instead of 'else' in expression statement"},
