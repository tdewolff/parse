...
func TestParse(t *testing.T) {
	var tests = []struct {
		...
		{"`tmpl`", "Stmt(`tmpl`)"},
		{"`tmpl${x}`", "Stmt(`tmpl${x}`)"},
		{"`tmpl` x `tmpl`", "Stmt(`tmpl`) Stmt(x`tmpl`)"},
		...
		{"yield a = 5", "Stmt(yield) Stmt(a=5)"},
		{"yield * a = 5", "Stmt((yield*a)=5)"},
		{"yield\na = 5", "Stmt(yield) Stmt(a=5)"},
		{"yield yield a", "Stmt(yield) Stmt(yield) Stmt(a)"},
		...
		{"x = (a) => a++", "Stmt(x=(Params(Binding(a)) => Stmt({ Stmt(a++) })))"},
		{"x = (a) => {a++}", "Stmt(x=(Params(Binding(a)) => Stmt({ Stmt(a++) })))"},
		{"x = (a,b) => {a++}", "Stmt(x=(Params(Binding(a) , Binding(b)) => Stmt({ Stmt(a++) })))"},
