...
func (p *Parser) parseStmt() (stmt IStmt) {
	...
	case LetToken:
		let := p.data
		p.next()
		if IsIdentifier(p.tt) || p.tt == YieldToken || p.tt == AwaitToken || p.tt == OpenBracketToken || p.tt == OpenBraceToken {
			varDecl := p.parseVarDecl(tt)
			stmt = &varDecl
		} else {
			// expression
	...
	case AsyncToken: // async function
		...
		if p.tt == FunctionToken && !p.prevLT {
			...
			// expression
			stmt = &ExprStmt{p.parseAsyncExpression(OpExpr)}
			if !p.prevLT && p.tt != SemicolonToken && p.tt != CloseBraceToken && p.tt != ErrorToken {
				p.fail("expression")
			}
		}
	...
	default:
		if p.isIdentifierReference(p.tt) {
			...
			if p.tt == ColonToken {
				p.next()
				stmt = &LabelledStmt{label, p.parseStmt()}
			} else {
				// expression
	...
	if p.tt == MulToken || p.tt == OpenBraceToken {
		...
		if p.tt == FunctionToken {
			...
			if p.tt == FunctionToken && !p.prevLT {
				...
				// expression
				exportStmt.Decl = p.parseAsyncExpression(OpExpr)
			}
		} else if p.tt == ClassToken {
			classDecl := p.parseClassDecl()
			exportStmt.Decl = &classDecl
...
func (p *Parser) parseAnyFunc(async, inExpr bool) (funcDecl FuncDecl) {
	...
	if inExpr && (IsIdentifier(p.tt) || p.tt == YieldToken || p.tt == AwaitToken) || !inExpr && p.isIdentifierReference(p.tt) {
		name = p.data
		if !inExpr {
			funcDecl.Name, ok = p.scope.Declare(p.ctx, VariableDecl, p.data)
			if !ok {
...
func (p *Parser) parseAnyClass(inExpr bool) (classDecl ClassDecl) {
	...
	if IsIdentifier(p.tt) || p.tt == YieldToken || p.tt == AwaitToken {
		if !inExpr {
			var ok bool
			classDecl.Name, ok = p.scope.Declare(p.ctx, LexicalDecl, p.data)
			if !ok {
				p.failMessage("identifier '%s' has already been declared", string(p.data))
				return
			}
		...
		if p.tt == EllipsisToken {
			...
			if p.tt == OpenParenToken {
				...
				if p.assumeArrowFunc {
					property.Value, _ = p.scope.Declare(p.ctx, ArgumentDecl, method.Name.Literal.Data) // cannot fail
				} else {
...
func (p *Parser) parseIdentifierExpression(prec OpPrec, ident []byte) IExpr {
	var left IExpr
	left = p.scope.Use(p.ctx, ident)
	return p.parseExpressionSuffix(left, prec, OpPrimary)
}

func (p *Parser) parseAsyncExpression(prec OpPrec) IExpr {
	var left IExpr
	precLeft := OpPrimary
	if p.tt == FunctionToken {
		// primary expression
		...
		return nil
	} else if !p.prevLT && (p.tt == OpenParenToken || IsIdentifier(p.tt) || !p.generator && p.tt == YieldToken) {
		// async arrow function expression
		arrowFunc := p.parseAsyncArrowFunc()
		left = &arrowFunc
		precLeft = OpAssign
	} else {
		left = p.scope.Use(p.ctx, asyncBytes)
	}
...
func (p *Parser) parseExpression(prec OpPrec) IExpr {
	...
	case OpenParenToken:
		...
		left, precLeft = p.parseParenthesizedExpressionOrArrowFunc()
	...
	case AsyncToken:
		p.next()
		left = p.parseAsyncExpression(prec)
	case ClassToken:
...
func (p *Parser) parseAssignmentExpression() IExpr {
	...
	if p.assumeArrowFunc && IsIdentifier(p.tt) {
		...
		if p.tt == EqToken || p.tt == CommaToken || p.tt == CloseParenToken || p.tt == CloseBraceToken || p.tt == CloseBracketToken {
			var left IExpr
			left, _ = p.scope.Declare(p.ctx, ArgumentDecl, data) // cannot fail
			return p.parseExpressionSuffix(left, OpAssign, OpPrimary)
		}
		p.assumeArrowFunc = false
		if tt == AsyncToken {
			return p.parseAsyncExpression(OpAssign)
		}
...
func (p *Parser) parseParenthesizedExpressionOrArrowFunc() (left IExpr, precLeft OpPrec) {
	// expect to be at (
	p.next()
	...
	for {
		if p.tt == ErrorToken {
			...
			p.next()
			break
		} else if p.tt == EllipsisToken && p.assumeArrowFunc {
			p.next()
	...
	if p.tt == ArrowToken && p.assumeArrowFunc {
		...
		arrowFunc.Scope.UndeclareScope(p.ctx)
